...
const btnSubmit = document.querySelectorAll('.js-submit');
...
calcForm.addEventListener('input', () => {
  btnSubmit.disabled = !(calcForm.width.value > 0 && calcForm.lenght.value > 0);
});
...
validator
  ...
  .then((data) => {
    form.reset ();
    alert(`Спасибо! Ваша заявка под номером: ${data.id}`)
  });
  })